...
func mkdisk(commandArray []string) {
	...
	for i := 1; i < len(commandArray); i++ {
		...
		// Se usa ToLower para pasar a minusculas
		...
		if band_path {
			...
			if band_size {
				...
				if band_fit {
					...
					copy(master_boot_record.Dsk_fit[:], "ff")
				}
...
func rmdisk(commandArray []string) {
	...
	if !band_error {
		// Verifico que el parametro "Path" (Obligatorio)
		for band_path {
				...
				if index_p != -1 {
					...
					if err == nil {
						...
						if Mount.Buscar_particion(val_path, val_name, lista_montajes) {
							...
							//Carnet (los ultimos dos digitos)
...
func crear_particion_primaria(direccion string, nombre string, size int, fit string, unit string) {
	...
	if err != nil {
		...
		// Calcular el tamaño de struct en bytes
		mbr2 := struct_a_bytes(mbr_empty)
		sstruct := len(mbr2)
					...
					if !existe_particion(direccion, nombre) {
						...
						if s_dsk_fit == "ff" {
							...
							for i := 0; i < 4; i++ {
								...
								if s_part_start_act == "-1" || (s_part_status_act == "1" && i_part_size_act >= size_bytes) {
									if i != num_particion {
										// Obtengo el tamaño de la particion del peor indice
